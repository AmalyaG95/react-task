...
const Results = () => {
  ...
  const rightAnswers = answers.filter((answer, index) => answer.text === questions[index]?.answer);
  console.log("🚀 ~ file: index.jsx ~ line 12 ~ Results ~ rightAnswers", rightAnswers)
    ...
    <div className={styles.container}>
      ...
      <h3>Your score is {rightAnswers.length} / 10</h3>
      {
        ...
        <div key={id}>
          ...
          <p style={{color: `${answers[index]?.text === answer? 'green' : 'red'} `}}>{answers[index]?.text === answer ? 'Right' : 'Wrong'} answer</p>
        </div>
        ))
      }
      <Link to="/" onClick={handleClick}>Try again</Link>
    </div>
  )
}
